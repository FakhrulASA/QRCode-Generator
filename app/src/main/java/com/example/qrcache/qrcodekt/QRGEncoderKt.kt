...
class QRGEncoderKt {
    ...
    constructor(data: String?, type: String?) {
        encoded = encodeContents(data, null, QRGContentsKt.Type.TEXT)
    }
    ...
    private fun encodeQRCodeContents(data: String?, bundle: Bundle?, type: String) {
        var data = data
        when (type) {
            QRGContentsKt.Type.TEXT -> if (data != null && data.length > 0) {
                contents = data
                displayContents = data
                title = "Text"
            }
            QRGContentsKt.Type.EMAIL -> {
                data = trim(data)
                if (data != null) {
            ...
            QRGContentsKt.Type.CONTACT -> if (bundle != null) {
                ...
                val uniquePhones: MutableCollection<String> = HashSet(QRGContentsKt.PHONE_KEYS.size)
                run {
                    var x = 0
                    while (x < QRGContentsKt.PHONE_KEYS.size) {
                        val phone =
                            trim(bundle.getString(QRGContentsKt.PHONE_KEYS[x]))
                        if (phone != null) {
                ...
                val uniqueEmails: MutableCollection<String> = HashSet(QRGContentsKt.EMAIL_KEYS.size)
                var x = 0
                while (x < QRGContentsKt.EMAIL_KEYS.size) {
                    val email = trim(bundle.getString(QRGContentsKt.EMAIL_KEYS[x]))
                    if (email != null) {
                ...
                val url = trim(bundle.getString(QRGContentsKt.URL_KEY))
                if (url != null) {
                    // escapeVCard(url) -> wrong escape e.g. http\://zxing.google.com
                    newContents.append("URL:").append(url).append(';')
                    newDisplayContents.append('\n').append(url)
                }
                val note = trim(bundle.getString(QRGContentsKt.NOTE_KEY))
                if (note != null) {
            ...
            QRGContentsKt.Type.LOCATION -> if (bundle != null) {
